...
export class UserFormComponent implements OnInit {
  ...
  constructor(private _userService: UserService,
    private _rotuer: Router, public authService: AuthService, private formBuilder: FormBuilder) { }
        ...
        this._userService.getUsers().subscribe((users) => {
          ...
          for (var i = 0; i < users.length; i++) {
            if (this.user.username == this.users[i].username) {
              ...
              this._rotuer.navigate(['/hp']);
            } else if (this.user.username == this.users[i].email) {
              localStorage.setItem('isLoggedIn', "true");
              localStorage.setItem('token', this.users[i].username);
              this._rotuer.navigate(['/hp']);
            }
          }
        });
